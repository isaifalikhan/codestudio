 ...
 *
 * The template produced five near-identical paragraphs for every tool with only
 * the name swapped in, which reads as scaled/spun content to both Google's spam
 * systems and AdSense's content review. Everything here is specific to the
 * individual tool: what it actually does, the tradeoff or gotcha worth knowing,
 * and what people realistically use it for.
 */

export const EXTENDED_LONG_DESCRIPTIONS: Record<string, string> = {
  // ── IMAGE TOOLS ───────────────────────────────────────────
...
export const EXTENDED_HOW_TO_STEPS: Record<string, [string, string, string]> = {
  'image-converter': [
